...
---
flowchart TD
    A["Private Property Rights"] -- Precondition --> B["Exchange"]
    B -- Precondition --> C["Specialization"]
    A@{ shape: rounded}
    B@{ shape: rounded}
    C@{ shape: rounded}
```
...
---
flowchart TD
    A["Scarcity"] --> B["Competition"]
    B --> C1(["Price"]) & C2(["Non-price"])
    C1 --> D1["Winners"]
    C1 -- discrimination --> D2["Losers (Discriminated)"]
    C2 --> D1
    C2 -- discrimination --> D2
    A@{ shape: rounded}
    B@{ shape: rounded}
    D1@{ shape: hex}
    D2@{ shape: hex}
...
```mermaid
flowchart TD
	A[(Standardised tasks/ quality of output can be easily measured?)]
	B(Piece Rate)
	C(Time Rate)
	A -- Yes --> B
	A -- No --> C 
